...
def QTGMC_obs(
	...
	if SVThin > 0:
		...
		vertMedD = core.std.Expr([lossed1, lossed1.zsmooth.VerticalCleaner(mode=[1] if isGray else [1, 0])], expr=[expr] if isGray else [expr, ''])
...
def QTGMC_obs_MakeLossless(Input, Source, InputType, TFF):
	...
	vmNewDiff2 = core.std.Expr([vmNewDiff1.zsmooth.VerticalCleaner(mode=[1]), vmNewDiff1], expr=[expr])
